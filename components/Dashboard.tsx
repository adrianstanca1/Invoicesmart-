...
import { Invoice, FinancialInsight, Client, Transaction, TaxRule } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ invoices, clients, transactions, taxRules }) => {
  const [insight, setInsight] = useState<FinancialInsight | null>(null);
  const [loading, setLoading] = useState(false);

  const stats = useMemo(() => {
    const totalRevenue = invoices.reduce((sum, inv) => 
      sum + inv.lineItems.reduce((s, item) => s + (item.quantity * item.rate), 0), 0
    );
  ...
  const chartData = useMemo(() => {
    ...
    invoices.forEach(inv => {
      ...
      const amount = inv.lineItems.reduce((s, item) => s + (item.quantity * item.rate), 0);
  ...
  return (
    <div className="p-6 space-y-8 animate-fade-in">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-bold text-slate-800">Financial Dashboard</h2>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <div className="text-sm text-slate-500 mb-1">Total Revenue (Invoiced)</div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="text-3xl font-bold text-green-600">£{stats.paidAmount.toLocaleString()}</div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className={`text-3xl font-bold ${stats.netProfit >= 0 ? 'text-slate-900' : 'text-red-600'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 h-80">
          <h3 className="text-lg font-semibold mb-4 text-slate-700">Revenue Stream</h3>
          {chartData.length > 0 ? (
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 p-6 rounded-xl border border-indigo-100 relative overflow-hidden">
           ...
           <h3 className="text-lg font-semibold mb-4 text-indigo-900 flex items-center gap-2">
             <i className="fas fa-robot text-indigo-600"></i> AI Accountant Insights
           </h3>
           ...
           {insight ? (
             <div className="space-y-4 relative z-10">
               ...
               <div>
                 <p className="text-sm font-medium text-indigo-900 mb-2">Recommendations</p>
               ...
               <div className="bg-red-50 p-3 rounded-lg border border-red-100">
                 <p className="text-sm font-medium text-red-800">Risk Assessment</p>
                 <p className="text-red-600 text-sm mt-1">{insight.riskAssessment}</p>
               </div>
             </div>
           ) : (
             <div className="h-48 flex flex-col items-center justify-center text-center relative z-10">
               <p className="text-indigo-400 mb-2">Click "Analyze with AI" to get personalized financial advice based on your invoices.</p>
             </div>
           )}
        </div>
      </div>
